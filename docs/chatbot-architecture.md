...
A `MessageWindowChatMemory` backed by an `InMemoryChatMemoryRepository` stores the last N messages per session:
...
- **Window size**: configurable via `petclinic.chat.memory.window-size` in `application.properties` (default 20)
- **Lifecycle**: in-process only; memory clears on application restart
- **Future enhancement**: swap `InMemoryChatMemoryRepository` for a JDBC-backed repository if persistence across restarts is needed
